/**
 * End-to-end integration test with real OpenAI API
 *
 * This test validates the complete workflow:
 * 1. Gateway connects to simple-test-server MCP server
 * 2. Tools are retrieved in OpenAI format
 * 3. OpenAI API is called with tools
 * 4. OpenAI generates function calls
 * 5. Function calls are executed via gateway
 * 6. Results are returned to OpenAI
  ...
  try {
    ...
    if (fs.existsSync(rootEnvPath)) {
      ...
      for (const line of text.split(/\r?\n/)) {
        const m = line.match(/^\s*(?:export\s+)?OPENAI_API_KEY\s*=\s*['\"]?([^'\"\n]+)['\"]?\s*$/)
        if (m) {
          OPENAI_API_KEY = m[1]
...
const describeOrSkip = OPENAI_API_KEY ? describe : describe.skip

describeOrSkip('OpenAI E2E Integration', () => {
  ...
  beforeAll(async () => {
    ...
    process.env.MCP_BASE_PATH = path.join(process.cwd(), '.tmp', 'openai-e2e-test')
    process.env.MCP_LOG_PATH = path.join(process.cwd(), '.tmp', 'openai-e2e-test', 'mcp-calls.log')
  ...
  it('completes full workflow: tool discovery → OpenAI call → execution → response', async () => {
    ...
    // Step 2: Test simple math operation with OpenAI
    console.log('\n=== Testing Math Operation with OpenAI ===')
    ...
    console.log('OpenAI initial response:', mathResponse.choices[0].message.content || '(function call)')

    // Step 3: Check if OpenAI made a function call
    ...
    // Step 5: Send result back to OpenAI
    ...
    console.log('OpenAI final response:', finalResponse.choices[0].message.content)
